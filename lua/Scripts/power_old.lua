...
-- PIDs structures
-- controls NP at 70.8% at beta with EEC on
local eec_mixture_pid = {
...
-- controls torque at alpha with EEC on
local eec_throttle_pid = {
...
-- controls NH at beta with EEC off
local nh_mixture_pid = {
	[0] = { ["kp"] = 0.2, ["ki"] = 0.15, ["kd"] = 0, ["ts"] = 0.1, ["cv_min"] = 0.4, ["cv_max"] = 1, ["cv_up"] = 0.2, ["cv_dw"] = 0.2 },
	[1] = { ["kp"] = 0.2, ["ki"] = 0.15, ["kd"] = 0, ["ts"] = 0.1, ["cv_min"] = 0.4, ["cv_max"] = 1, ["cv_up"] = 0.2, ["cv_dw"] = 0.2 }
}
-- controls NH at alpha with EEC off
local nh_throttle_pid = {
	[0] = { ["kp"] = 0.02, ["ki"] = 0.015, ["kd"] = 0, ["ts"] = 0.1, ["cv_min"] = 0, ["cv_max"] = 1, ["cv_up"] = 0.2, ["cv_dw"] = 0.2 },
	[1] = { ["kp"] = 0.02, ["ki"] = 0.015, ["kd"] = 0, ["ts"] = 0.1, ["cv_min"] = 0, ["cv_max"] = 1, ["cv_up"] = 0.2, ["cv_dw"] = 0.2 }
}
...
function eec(ind)
	...
	if pla < 13 then
		...
		if eng_eec[ind] == 1 and cla > 25.7 and eng_autofeather[ind] ~= 2 then
			--EEC on, prop not feathered
			temp_eng_throttle = -2.0 - ( ( -1.0 / 13 ) * pla )
		else		
			--EEC off, prop feathered (hotel mode)
			-- get commanded engine NH
			local eng_nh_ratio = lookup_table(pla, base_pla)
			
			-- PID to set engine throttle to produce NH
			nh_throttle_pid[ind]["cv"] = eng_throttle[ind]
			nh_throttle_pid[ind]["pv"] = eng_nh[ind]
			nh_throttle_pid[ind]["sp"] = eng_nh_ratio * 100
			nh_throttle_pid[ind]["ff"] = ( eng_nh_ratio - 0.7 ) * 0.3
			pid.run(nh_throttle_pid[ind])
			temp_eng_throttle = -1 - nh_throttle_pid[ind]["cv"]
		end
	...
	elseif pla < 37 then
		...
		if eng_eec[ind] == 1 and cla > 25.7 and eng_autofeather[ind] ~= 2 then
			...
			eec_mixture_pid[ind]["cv"] = eng_mixture[ind]
			eec_mixture_pid[ind]["pv"] = prop_speed_act[ind]
			eec_mixture_pid[ind]["sp"] = prop_speed_max * 0.708
			pid.run(eec_mixture_pid[ind])
			temp_eng_mix = eec_mixture_pid[ind]["cv"]
		else
			--EEC off, prop feathered (hotel mode)
			...
			-- PID to set engine mixture to produce NH
			nh_mixture_pid[ind]["cv"] = eng_throttle[ind]
			nh_mixture_pid[ind]["pv"] = eng_nh[ind]
			nh_mixture_pid[ind]["sp"] = eng_nh_ratio * 100
			nh_mixture_pid[ind]["ff"] = ( eng_nh_ratio - 0.7 ) * 0.3
			pid.run(nh_mixture_pid[ind])
			temp_eng_mix = nh_mixture_pid[ind]["cv"]
		end
		--reset EEC frozen (as bellow 52 PLA)
		eng_throttle_last[ind] = 0
	else
		...
		if eng_eec[ind] == 1 and cla > 25.7 and eng_autofeather[ind] ~= 2 then
			...
			eec_throttle_pid[ind]["cv"] = eng_throttle[ind]
			eec_throttle_pid[ind]["pv"] = eng_power[ind]
			eec_throttle_pid[ind]["sp"] = eng_power_ratio * eng_power_max
			eec_throttle_pid[ind]["ff"] = eng_power_ratio / 1.15
			pid.run(eec_throttle_pid[ind])
			temp_eng_throttle = eec_throttle_pid[ind]["cv"]
			...
			if eng_throttle_last[ind] == 0 then
				...
				nh_throttle_pid[ind]["ff"] = ( eng_nh_ratio - 0.7 ) * 0.3
				pid.run(nh_throttle_pid[ind])
				temp_eng_throttle = nh_throttle_pid[ind]["cv"]
			else
				--EEC frozen
